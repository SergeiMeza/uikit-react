                        ...
                        : '') }, { children: _jsx("nav", __assign({ className: classNames(type === TabNavigationType.default
                            ...
                            : 'flex space-x-4'), "aria-label": "Tabs" }, { children: tabItems.map(function (tab) { return (_jsxs(Anchor, __assign({ type: "button", to: tab.to, href: tab.href, onClick: function () {
                                ...
                                ? classNames(tab.current
                                    ...
                                    : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300', 'group inline-flex items-center', 'py-4 px-1 border-b-2 font-medium text-sm', tabItemSelectedClassNames)
                                : classNames(tab.current
                                    ? 'bg-primary-100 text-primary-700'
                                    : 'text-gray-500 hover:text-gray-700', 'px-3 py-2 font-medium text-sm rounded-md', tabItemClassNames), "aria-current": tab.current ? 'page' : undefined }, { children: [showIcons && tab.icon && (_jsx(tab.icon, { className: classNames(tab.current
                                        ? 'text-primary-500'
